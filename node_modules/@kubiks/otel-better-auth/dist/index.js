...
const INSTRUMENTED_FLAG = "__kubiksOtelBetterAuthInstrumented";
 ...
 */
function endSpanWithSuccess(span) {
 ...
 */
function wrapAuthMethod(originalFn, spanName, attributes, tracer) {
                ...
                if (result) {
                    if (result.data) {
                        ...
                        if (result.error) {
                            ...
                            span.setAttribute(SEMATTRS_AUTH_ERROR, result.error.message || "Unknown error");
                            span.setStatus({ code: SpanStatusCode.ERROR });
                            span.end();
                            return result;
                        }
 ...
 */
function instrumentServer(server, tracer) {
    ...
    for (const [methodName, metadata] of Object.entries(API_METHOD_METADATA)) {
        if (typeof api[methodName] === "function") {
            ...
            api[methodName] = wrapAuthMethod(originalMethod, spanName, attributes, tracer);
    ...
    for (const key of Object.keys(api)) {
        if (typeof api[key] === "function" &&
            ...
            api[key] = wrapAuthMethod(originalMethod, spanName, { [SEMATTRS_AUTH_OPERATION]: operationName }, tracer);
        }
    }
    return server;
}
function ensureOtelPlugin(auth, config) {
    const authWithOptions = auth;
    ...
    const pluginsArray = Array.isArray(existingPlugins)
        ? existingPlugins
        : existingPlugins &&
            typeof existingPlugins[Symbol.iterator] === "function"
            ? Array.from(existingPlugins)
            : [];
    const hasOtelPlugin = pluginsArray.some((plugin) => plugin && typeof plugin === "object" && plugin.id === "otel");
    if (hasOtelPlugin) {
 ...
 * ```
 */
export function instrumentBetterAuth(auth, config) {
    if (!auth || typeof auth !== "object") {
        return auth;
    }
    if (auth[INSTRUMENTED_FLAG]) {
        return auth;
    }
    const { tracerName = DEFAULT_TRACER_NAME, tracer: customTracer } = config ?? {};
    const tracer = customTracer ?? trace.getTracer(tracerName);
    ensureOtelPlugin(auth, { tracerName, tracer });
    instrumentServer(auth, tracer);
    auth[INSTRUMENTED_FLAG] = true;
    return auth;
}
/**
 ...
 * ```
 */
export function otelPlugin(config) {
    const { tracerName = DEFAULT_TRACER_NAME, tracer: customTracer } = config ?? {};
    const tracer = customTracer ?? trace.getTracer(tracerName);
    return {
        id: "otel",
